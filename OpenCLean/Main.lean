...
@[blueprint "lem:mangoldt-dirichlet-series-finite-threshold-geometric-bound-local"
  (statement := /-- For every real number $u>0$ and every natural cutoff $N$,
  the finite sum of $\Lambda(q)q^{-1-u}$ over natural $q<N$ with $q\geq2$ is
  at most $\log 2/(2^u-1)$. -/)
  (proof := /-- By
  \cref{lem:mangoldt-dirichlet-series-summable-local}, the full real von
  Mangoldt Dirichlet series is summable.  The selected finite terms are
  non-negative, so the selected finite sum is at most the full t-sum.  By
  \cref{lem:mangoldt-dirichlet-series-eq-zeta-log-derivative}, this t-sum is
  the real part of the negative logarithmic derivative of $\zeta$ at $1+u$,
  and \cref{lem:zeta-log-derivative-geometric-bound} bounds that real part by
  $\log 2/(2^u-1)$. -/)
  (title := /-- Finite thresholded Dirichlet sums with geometric constant -/)
  (latexEnv := "lemma")]
...
@[blueprint "lem:two-power-kernel-midpoint-bound-local"
  (statement := /-- For every real number $t>0$, the elementary kernel
  satisfies
  $$ {t\log 2\over 2^t-1}\leq 2^{-t/2}. $$ -/)
  (proof := /-- Put $s=t\log 2>0$.  The inequality is equivalent, after
  multiplying by the positive denominator, to
  $s\leq e^{s/2}-e^{-s/2}$.  Since $s/2\geq0$, Mathlib's hyperbolic-sine
  inequality gives $s/2\leq\sinh(s/2)$, and the identity
  $2\sinh a=e^a-e^{-a}$ gives the desired bound. -/)
  (title := /-- Midpoint bound for the two-power kernel -/)
  (latexEnv := "lemma")]
...
@[blueprint "lem:mangoldt-tail-prime-range-slack-local"
  (statement := /-- For every natural $p\geq2$ and every natural cutoff $N$,
  the finite ordinary Mangoldt tail at $p$ is bounded by
  $1/(\log p+\log 2/2)$. -/)
  (proof := /-- Rewrite each selected summand by
  \cref{lem:mangoldt-tail-term-integral-local} and exchange the finite sum with
  the integral, using \cref{lem:log-square-integral-kernel-integrable-local} for
  integrability.  The finite integrand factors by
  \cref{lem:mangoldt-tail-integrand-factor-local}.  For each $t>0$,
  \cref{lem:mangoldt-dirichlet-series-finite-threshold-geometric-bound-local}
  bounds the finite Dirichlet sum by $\log 2/(2^t-1)$, and
  \cref{lem:two-power-kernel-midpoint-bound-local} bounds the remaining kernel
  by $2^{-t/2}$.  Thus the integrand is dominated by
  $e^{-(\log p+\log2/2)t}$, whose integral over $(0,\infty)$ is
  $1/(\log p+\log2/2)$. -/)
  (title := /-- Finite prime-tail slack bound -/)
  (latexEnv := "lemma")]
...
@[blueprint "lem:mangoldt-tail-sharp-prime-slack"
  ...
  finite-range estimate \cref{lem:mangoldt-tail-prime-range-slack-local} bounds
  every partial ordinary tail sum by $1/(\log p+\log2/2)$.  Since the selected
  summands are non-negative, bounded partial sums give summability of the full
  tail and the same bound for its t-sum.  Multiplying by $\log p>0$ and
  rewriting $\log p/(\log p+\log2/2)$ as $x/(x+1/2)$ gives the displayed
  inequality. -/)
...
@[blueprint "def:eps-adjoint-kernel-package"
  ...
  non-negative, has finite natural-row partial sums bounded by $1$, has total
  mass one from every natural state $n\geq2$ after adding the absorbing slack
  term, sends $\infty$ to itself, moves between natural states only upward along
...
@[blueprint "lem:eps-adjoint-kernel-package-from-subinvariant"
  ...
  $q>1$.  Reindexing any finite natural outgoing partial sum by the corresponding
  non-zero incoming terms rewrites it as a finite sub-sum of
...
@[blueprint "def:mangoldt-von-mangoldt-downward-kernel-invariant"
  (statement := /-- For a kernel $P:\mathbb N\times\mathbb N\to\mathbb R$, this
  predicate is the fixed formal interface for the von Mangoldt downward chain.
  The kernel has non-negative rows of total mass one on positive states, the
  state $1$ is absorbing, non-zero transitions from a state $n\geq2$ move
  strictly downward in the divisibility poset, the non-absorbing transition from
  $n$ to $n/q$ has weight $\Lambda(q)/\log n$, and the invariant weight
  \cref{def:mangoldt-weight} satisfies the incoming recurrence for every
  positive state. -/)
  (title := /-- Invariant von Mangoldt downward-kernel interface -/)
  (latexEnv := "definition")]
noncomputable def mangoldt_von_mangoldt_downward_kernel_invariant
    (P : ℕ → ℕ → ℝ) : Prop :=
  (∀ n m : ℕ, 0 ≤ P n m) ∧
    (∀ n : ℕ, 1 ≤ n -> (∑' m : ℕ, P n m) = 1) ∧
    P 1 1 = 1 ∧
    (∀ m : ℕ, m ≠ 1 -> P 1 m = 0) ∧
    (∀ n m : ℕ, 2 ≤ n -> P n m ≠ 0 -> m ∣ n ∧ m < n) ∧
    (∀ n q : ℕ, 2 ≤ n -> 1 < q -> q ∣ n ->
      P n (n / q) = ArithmeticFunction.vonMangoldt q / Real.log (n : ℝ)) ∧
    (∀ n : ℕ, 1 ≤ n ->
...
@[blueprint "def:mangoldt-adjoint-kernel-package"
  ...
  predicate says that $U$ is the adjoint upward kernel obtained from the
  von Mangoldt downward kernel $P$ and the invariant weight
  \cref{def:mangoldt-weight}.  It includes the downward invariant-kernel
  interface \cref{def:mangoldt-von-mangoldt-downward-kernel-invariant},
  non-negativity and unit mass of the upward rows, vanishing of diagonal
  transitions, the adjoint formula
  $U(n,m)=\nu_\Lambda(m)\nu_\Lambda(n)^{-1}P(m,n)$ off the diagonal, and the
  upward divisibility support condition. -/)
  (title := /-- Adjoint von Mangoldt upward-kernel package -/)
  (latexEnv := "definition")]
noncomputable def mangoldt_adjoint_kernel_package (P U : ℕ → ℕ → ℝ) : Prop :=
  mangoldt_von_mangoldt_downward_kernel_invariant P ∧
    (∀ n m : ℕ, 0 ≤ U n m) ∧
    (∀ n : ℕ, 1 ≤ n -> (∑' m : ℕ, U n m) = 1) ∧
    (∀ n : ℕ, U n n = 0) ∧
    (∀ n m : ℕ, 1 ≤ n -> 1 ≤ m -> m ≠ n ->
...
@[blueprint "def:mangoldt-adjoint-kernel-markov-law"
  ...
  process $p$, this predicate says that $\mu$ is governed by the one-step
  Markov transition law with kernel $U$: for every time $k$ and states $n,m$,
  the mass of paths with $p_k=n$ and $p_{k+1}=m$ is the mass of paths with
  $p_k=n$ multiplied by $U(n,m)$. -/)
  (title := /-- One-step Markov law for an adjoint kernel path -/)
  (latexEnv := "definition")]
noncomputable def mangoldt_adjoint_kernel_markov_law {Ω : Type} [MeasurableSpace Ω]
...
@[blueprint "def:mangoldt-adjoint-kernel-path-data"
  ...
  one-step Markov law \cref{def:mangoldt-adjoint-kernel-markov-law}. -/)
...
@[blueprint "def:mangoldt-adjoint-kernel-analytic-path-data"
  (statement := /-- This predicate records the analytic information supplied by
  the adjoint von Mangoldt trajectory construction beyond the adjacent Markov
  cylinder law: the hit counts satisfy the uniform second-moment estimate
  \cref{def:mangoldt-adjoint-second-moment-bound}, and the same trajectory law
  satisfies the reverse-Fatou pathwise extraction principle
  \cref{def:mangoldt-adjoint-reverse-fatou-extraction-principle}. -/)
  (title := /-- Analytic data for the adjoint von Mangoldt path law -/)
  (latexEnv := "definition")]
noncomputable def mangoldt_adjoint_kernel_analytic_path_data {Ω : Type}
    [MeasurableSpace Ω] (μ : MeasureTheory.Measure Ω) (path : Ω → ℕ → ℕ) : Prop :=
...
@[blueprint "lem:mangoldt-adjoint-kernel-path-data-exists"
  ...
  trajectory sample space, a probability measure, and a natural-valued path
  process satisfying both the Markov path-data predicate
  \cref{def:mangoldt-adjoint-kernel-path-data} and the analytic path-data
  predicate \cref{def:mangoldt-adjoint-kernel-analytic-path-data}. -/)
  ...
  \cref{def:mangoldt-adjoint-kernel-path-data}.  The pathwise predecessor-count
  estimate and the reverse-Fatou argument in the adjoint von Mangoldt proof give
  the second-moment and extraction clauses collected in
  \cref{def:mangoldt-adjoint-kernel-analytic-path-data}. -/)
    ...
    ∃ (P : ℕ → ℕ → ℝ) (U : ℕ → ℕ → ℝ) (Ω : Type) (mΩ : MeasurableSpace Ω)
      (μ : MeasureTheory.Measure Ω) (path : Ω → ℕ → ℕ),
        mangoldt_adjoint_kernel_package P U ∧
          @mangoldt_adjoint_kernel_path_data Ω mΩ U μ path ∧
            @mangoldt_adjoint_kernel_analytic_path_data Ω mΩ μ path := by
...
@[blueprint "def:mangoldt-adjoint-random-model"
  ...
  \cref{def:mangoldt-adjoint-visit-identity}, the hit counts satisfy the
  asymptotic second-moment estimate
  \cref{def:mangoldt-adjoint-second-moment-bound}, and the model satisfies the
...
@[blueprint "def:mangoldt-adjoint-constructed-path-data"
  ...
  estimate \cref{def:mangoldt-adjoint-second-moment-bound}, together with the
  reverse-Fatou pathwise extraction principle
  \cref{def:mangoldt-adjoint-reverse-fatou-extraction-principle}. -/)
...
@[blueprint "lem:mangoldt-adjoint-visit-identity-from-kernel-path-data"
  ...
  the expected visits of $p$ satisfy
...
@[blueprint "lem:mangoldt-adjoint-second-moment-bound-from-kernel-path-data"
  ...
  adjoint von Mangoldt kernel package
  \cref{def:mangoldt-adjoint-kernel-package} and $(U,\mu,p)$ satisfies the
  Markov path-data predicate \cref{def:mangoldt-adjoint-kernel-path-data}, and
  if the same trajectory law satisfies the analytic path-data predicate
  \cref{def:mangoldt-adjoint-kernel-analytic-path-data}, then the hit counts of
  $p$ satisfy the uniform second-moment estimate
  \cref{def:mangoldt-adjoint-second-moment-bound}. -/)
  (proof := /-- Assume the kernel package
  \cref{def:mangoldt-adjoint-kernel-package}, the Markov path-data predicate
  \cref{def:mangoldt-adjoint-kernel-path-data}, and the analytic path-data
  predicate \cref{def:mangoldt-adjoint-kernel-analytic-path-data}.  By the
  definition of the analytic path-data predicate, its first clause is precisely
  the uniform second-moment estimate
  \cref{def:mangoldt-adjoint-second-moment-bound} for $(\mu,p)$. -/)
...
@[blueprint "lem:mangoldt-adjoint-constructed-path-data-from-kernel-path-data"
  ...
  \cref{def:mangoldt-adjoint-kernel-package} and $(U,\mu,p)$ satisfies the
  adjoint Markov kernel path-data predicate
  \cref{def:mangoldt-adjoint-kernel-path-data}, and if $(\mu,p)$ satisfies the
  analytic path-data predicate
  \cref{def:mangoldt-adjoint-kernel-analytic-path-data}, then $(\mu,p)$
  satisfies the construction-data predicate
  ...
  \cref{def:mangoldt-adjoint-kernel-path-data} for $(U,\mu,p)$ and the analytic
  path-data predicate \cref{def:mangoldt-adjoint-kernel-analytic-path-data} for
  $(\mu,p)$.  The construction-data predicate
  \cref{def:mangoldt-adjoint-constructed-path-data} asks for probability mass
  one, pathwise strict divisibility, coordinate measurability, the
  expected-visit identity, the second-moment estimate, and the reverse-Fatou
  extraction principle.  The first three clauses are part of the Markov
  path-data predicate.  The expected-visit identity is supplied by
  \cref{lem:mangoldt-adjoint-visit-identity-from-kernel-path-data}; the
  second-moment estimate is supplied by
  \cref{lem:mangoldt-adjoint-second-moment-bound-from-kernel-path-data}; and the
  reverse-Fatou extraction clause is the second clause of the analytic path-data
  predicate.  Combining these six clauses gives
  \cref{def:mangoldt-adjoint-constructed-path-data}. -/)
...
@[blueprint "lem:mangoldt-adjoint-constructed-path-data-exists"
  ...
  process $p$ satisfying \cref{def:mangoldt-adjoint-kernel-path-data} and
  \cref{def:mangoldt-adjoint-kernel-analytic-path-data}, with $P$ and $U$
  ...
  path clauses, and supplies the analytic visit, second-moment, and
  reverse-Fatou clauses required by
  \cref{def:mangoldt-adjoint-constructed-path-data}. -/)
...
@[blueprint "lem:mangoldt-adjoint-random-model-from-constructed-path-data"
  ...
  the random-model predicate are the same conjunction of six clauses: total mass
  one, pathwise strict divisibility, coordinate measurability, the expected-visit
  identity, the second-moment estimate, and the reverse-Fatou extraction
  principle.  Therefore $h$ itself proves
  \cref{def:mangoldt-adjoint-random-model} for $(\mu,p)$. -/)
...
@[blueprint "lem:mangoldt-adjoint-reverse-fatou-extraction-principle-from-model"
  ...
  (proof := /-- Let $\Omega$ be a measurable space, let $\mu$ be a measure on
  $\Omega$, and let $p:\Omega\to(\mathbb N\to\mathbb N)$ be a path process.
  Assume that $(\mu,p)$ satisfies \cref{def:mangoldt-adjoint-random-model}.  By
  the definition of the random-model predicate, its final clause is exactly the
  reverse-Fatou pathwise extraction principle
  \cref{def:mangoldt-adjoint-reverse-fatou-extraction-principle} for $(\mu,p)$.
  Thus the asserted extraction principle follows directly from the model
  contract. -/)
...
@[blueprint "lem:mangoldt-adjoint-random-model-exists"
  ...
  witnesses folds the probabilistic, visit-identity, second-moment, and
  reverse-Fatou clauses into the random path model predicate
  \cref{def:mangoldt-adjoint-random-model}. -/)
